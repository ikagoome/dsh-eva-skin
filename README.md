...
- **Artifact diff viewer** — Codex-style: after each modification, click a produced-file chip (the file list at the end of a turn) to open a right-side EVA panel. The panel shows the file's current full content — every line in white monospace, only the removed lines tint red and the added lines green — fetched through the bundled `eva-files` server companion (loopback route reading the file from disk); a file that cannot be read falls back to the stitched hunk view. Close it with ✕, Esc, or a click anywhere outside the panel. A file the skin has no diff for keeps the default host opener.
...
- **View a change** — after the agent edits or creates files, the turn ends with produced-file chips; click one to open the diff panel on the right. The panel shows the file's current full content — every line in white, only the changed lines tinted red (removed) / green (added) — fetched through the `eva-files` companion route; when the file cannot be read (deleted, binary, oversized) it falls back to the hunk view. Close it with ✕, Esc, or a click on any area outside the panel.
