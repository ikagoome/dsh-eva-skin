...
- **产物 Diff 查看器** — 类似 Codex:每次修改后,点击聊天末尾的产物文件芯片,右侧弹出 EVA 风格面板,以完整内容展示该文件的实际改动——所有已知内容用白色等宽字体呈现,只有被删除的行标红、新增的行标绿(保留改动前后的上下文,多个改动块自动拼接),按 ✕、Esc 或点击面板外部任意区域关闭;没有 diff 数据的文件保留默认的宿主打开行为。
...
- **查看改动** — agent 修改或创建文件后,该轮末尾会出现产物文件芯片;点击任意一个,右侧弹出 diff 面板,以完整内容展示改动:未改动行白色、删除行红色、新增行绿色。按 ✕、Esc 或点击面板外部任意区域关闭。
